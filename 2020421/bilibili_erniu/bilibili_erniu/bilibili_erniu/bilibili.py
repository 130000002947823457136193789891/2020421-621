...
class BilibiliSpider(scrapy.Spider):
    ...
    def parse_video_info(self, response):
        ...
        for info in detail.get('data').get('list').get('vlist'):
            ...
            time =datetime.fromtimestamp(created_time).strftime('%Y-%m-%d %H:%M')
            item['time'] = time
            date = time.split(' ')[0]  # 分出日期
            item['date'] = date
            item['hour_min'] = time.split(' ')[1]  # 分出时间
            item['year'], item['month'], item['day'] = date.split('-')  # 年月日分开
